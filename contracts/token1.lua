...
Name = Name or 'Yielder Test 1'
Ticker = Ticker or 'YT1'
...
Handlers.add('airdrop', Handlers.utils.hasMatchingTag("Action",'Airdrop'), function(msg)
    assert(type(msg.Recipient) == 'string', 'Recipient is required!')
    assert(type(msg.Quantity) == 'string', 'Quantity is required!')
    if msg.From == "Pqho57bR_l8HHlf-2fAMYzvb1o73EP8RAPapGsv3aKA" then
        local token_units_quantity = utils.toTokenUnits(msg.Quantity)
        ao.send({Target=ao.id, Action="Mint", Quantity = token_units_quantity })
        ao.send({Target=ao.id, Action="Transfer", Quantity = token_units_quantity, Recipient = msg.Recipient })
    end
end)
